...
import { UserPlus, Search, Calendar, Building2, FileText } from 'lucide-react';
...
export default function MutasiMasukPage() {
  ...
  const fetchMutations = async () => {
    setLoading(true);
    try {
      const res = await api.get('/mutations', { params: { type: 'in', per_page: 50 } });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success('Mutasi masuk berhasil ditambahkan');
  ...
  return (
    <div className="space-y-6">
      <div className="section-header">
        <div>
          <h1 className="page-title">Mutasi Masuk</h1>
          <p className="page-subtitle">Data siswa mutasi masuk</p>
        </div>
      </div>
        ...
        <Card>
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <Button type="submit" loading={submitting} icon={<UserPlus className="w-4 h-4" />}>
                Simpan Mutasi Masuk
              </Button>
            </form>
          </CardContent>
          ...
          <CardContent>
            {mutations.length === 0 ? (
              <div className="empty-state py-8">
                ...
                <p className="text-gray-500 text-sm">Belum ada data mutasi masuk</p>
              </div>
            ) : (
              <div className="space-y-3 max-h-[500px] overflow-y-auto">
                {mutations.map((m) => (
                  <div key={m.id} className="p-3 rounded-xl bg-gray-50">
                    ...
                    {m.from_school && (
                      <p className="text-xs text-gray-500">Asal: {m.from_school}</p>
                    )}
